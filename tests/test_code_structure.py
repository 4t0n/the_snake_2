...
def test_apple_inherits_from_game_object():
    assert issubclass(the_snake2.Apple, the_snake2.GameObject), (
...
def test_snake_inherits_from_game_object():
    assert issubclass(the_snake2.Snake, the_snake2.GameObject), (
...
def test_elements_exist(element_type, element_name):
    assert hasattr(the_snake2, element_name), (
...
def test_vars_type(expected_type, var_name):
    assert isinstance(getattr(the_snake2, var_name, None), expected_type), (
...
def test_vars_are_functions(func_name):
    assert callable(getattr(the_snake2, func_name, None)), (
