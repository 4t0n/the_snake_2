...
for class_name in ('GameObject', 'Snake', 'Apple'):
    assert hasattr(the_snake2, class_name), (
...
def _create_game_object(class_name):
    try:
        return getattr(the_snake2, class_name)()
...
def modified_clock():
    ...
    original_clock = the_snake2.clock
    modified_clock_obj = _Clock(original_clock)
    the_snake2.clock = modified_clock_obj
    yield
    the_snake2.clock = original_clock
